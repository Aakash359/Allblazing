...
class Location extends Component {
    ...
    getAddress = async ({latitude, longitude}, submit = false) => {
        ...
        Axios.get(api)
            .then((res) => {
                console.log(
                    'GET DATA FROM LANG LONG',
                    res?.data?.results[0]?.formatted_address
                )
                this.setState(
                    {address: res?.data?.results[0]?.formatted_address},
                    async () => {
                        setUserAddress(this.state.address).then((a) => {
    ...
    onSubmit = async () => {
        ...
        axios
            .post(API.COMPLETE_PROFILE, payload, config)
            .then((response) => {
    ...
    render() {
        ...
        return (
            <View style={CommonStyles.container}>
                <ScrollView
                    style={[
                        ConnectUserTypeStyles.wrapper,
                        LocationStyles.container,
                    ]}>
                    <StepBar count={5} selected={[0, 1, 2, 3, 4]} />
                    <View
                        style={[
                            UsernameStyle.inputWrapper,
                            LocationStyles.inputWrapper,
                        ]}>
                        <Text
                            style={[
                                AuthStyle.selectText,
                                LocationStyles.locationText,
                            ]}>
